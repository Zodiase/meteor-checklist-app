...
export default connect(
  ...
  (state, ownProps) => {
    ...
    const isLoadingChecklistDocument = objectPath.get(
      state,
      ['data.checklists.documents', idOfChecklist, 'loading'],
      false,
    );
    const isChecklistDocumentLoaded = objectPath.get(
      state,
      ['data.checklists.documents', idOfChecklist, 'ready'],
      false,
    );
    const checklistDocument = objectPath.get(state, [
      'data.checklists.documents',
      idOfChecklist,
      'source',
    ]);
    const updateDateOfChecklistDocument = objectPath.get(
      state,
      ['data.checklists.documents', idOfChecklist, 'lastUpdated'],
      0,
    );
    const idOfNewlyCreatedChecklist = objectPath.get(state, [
      'ui.checklist.idOfNewlyCreatedChecklist',
    ]);
    const isNewlyCreatedChecklist = idOfChecklist === idOfNewlyCreatedChecklist;
    const isWaitingConfirmationOfNewStep = objectPath.get(
      state,
      ['ui.checklist.waitingConfirmationOfNewStep'],
      false,
    );
    const errorWhenCreatingNewStep = objectPath.get(
      state,
      ['ui.checklist.errorWhenCreatingNewStep'],
      null,
    );
  ...
  (dispatch, ownProps) => {
    ...
    return {
      markNewlyCreatedChecklistAsOpen: () => {
        dispatch({
          type: getAction(
            'ui.checklistTemplate.index.createNewChecklist.openedTheNewOne',
          ).type,
          idOfChecklist,
        });
      },
        ...
        dispatch({
          ...
          onDocumentUpdate: (document) => {
            defer(() =>
              dispatch({
                type: getAction(
                  'data.checklistTemplate.document.updateLocalCopy',
                ).type,
                idOfChecklist,
                document,
              }),
      ...
      stopSubscriptionOfChecklist: () => {
        dispatch({
          type: getAction(
            'data.checklistTemplate.document.terminateSubscription',
          ).type,
          idOfChecklist,
        });
      },
      ...
      addStepToChecklist: (stepObj) => {
        dispatch({
          type: getAction(
            'ui.checklistTemplate.document.createNewStep.startWaitingConfirmation',
          ).type,
        });
        ...
        addStepToChecklist.call(
          ...
          (error, response) => {
            dispatch({
              type: getAction(
                'ui.checklistTemplate.document.createNewStep.handleResponse',
              ).type,
              idOfChecklist,
              step: stepObj,
              error,
      ...
      acknowledgeErrorWhenCreatingNewStep: () => {
        dispatch({
          type: getAction(
            'ui.checklistTemplate.document.createNewStep.acknowledgeError',
          ).type,
        });
      },
    };
